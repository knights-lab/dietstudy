...
opts_chunk$set(echo = TRUE, fig.path = "Figs_ak/", dev = c("png", "pdf"), dpi = 300)
...
div <- div[rownames(div) %in% map_sample$X.SampleID, colnames(div) %in% map_sample$X.SampleID]

div_d <- as.dist(div)

tax_beta <- as.data.frame(as.matrix(div_d))
...
micro_order <- tax_dist %>% 
  group_by(UserName) %>% 
  dplyr::summarise(ave_cloud = mean(mean_beta_div, na.rm=T)) # ave_cloud is inverse variability = stability
# sort by mean
...
food_div <- ggplot() +
  ...
  #geom_smooth(data = plot_food_per_subject, aes(x = as.integer(UserName.x), y = food_mean_beta_div), method = "loess", color = "black") +
   geom_point(data = plot_food_per_subject, aes(x = UserName.x, y = food_mean_beta_div), color = "#64BAAA", size = 1, alpha = 0.7) +
   geom_boxplot(data = plot_food_per_subject, aes(x = UserName.x, y = food_mean_beta_div, fill = UserName.x), fill = NA, outlier.shape = NA, lwd = .25) +
...
tax_div <- ggplot()+
  ...
  #geom_smooth(data = plot_tax_per_subject, aes(x = as.integer(UserName.x), y = mean_beta_div), method = "loess", color = "black") +
   geom_point(data = plot_tax_per_subject, aes(x = UserName.x, y = mean_beta_div), color = "#5a2071", size =1, alpha = 0.7) +
  geom_boxplot(data = plot_tax_per_subject, aes(x = UserName.x, y = mean_beta_div, fill = UserName), fill = NA, outlier.shape = NA, lwd = .25) +
  theme_classic() +
  theme(axis.title.x = element_text(size = 12),
...
corr_plot <- ggplot(data = mean_food_test_no_soy, aes(y = ave_cloud, x = `mean(PD_whole_tree)`)) +
  ...
  annotate("text", x = 90, y = 0.4, label = paste("italic(rho) ==",signif(mytest$estimate, 2)), parse = T, size = 3)+
  annotate("text", x = 90, y = 0.35, label = paste("italic(p) ==", signif(mytest$p.value, 1)), parse = T, size = 3)+
...
corr_plot <- ggplot(data = mean_food_test, aes(y = ave_cloud, x = `mean(PD_whole_tree)`)) +
  ...
  ylab("Microbiome Stability") +
  xlab("Food Diversity, Faith's") +
  annotate("text", x = 28, y = 0.4, label = paste("italic(rho) ==",signif(mytest$estimate, 2)), parse = T, size = 3)+
  annotate("text", x = 28, y = 0.35, label = paste("italic(p) ==", signif(mytest$p.value, 1)), parse = T, size = 3)+
  theme(axis.text = element_text(size = 7),
        axis.title = element_text(size =9))
...
for (i in colnames(plot[3:ncol(plot)])) {
  ...
  spieces_plots[[i]] <- ggplot(data = plot, aes_string(x = i, y = "ave_cloud")) + 
           ...
           geom_point(color = "#5a2071", size = 3, alpha = 0.7) +

...
Figure 4: Will show the the correlation between dietary diversity and microbiome diversity

There is a significant correlation between microbiome stability and dietary alpha diversity, so make sure to show that.

Top panel: Microbime variability v. dietary alpha diveristy and dietary beta diversity
Bottom panel

Supplemental will be whatever doesn't fit nicely on the figure. Consider individual foods microbiome stability correlations etc.
...
```{r food_div, echo = F, include = F, fig.height = 4, fig.width =6}
...
food_div <- ggplot() +
  geom_boxplot(data = plot_food_per_subject, aes(x = UserName.x, y = food_mean_beta_div, fill = UserName.x), fill = "orange", outlier.shape = NA, lwd = .25) +
  ...
  geom_boxplot(data = plot_food_per_subject, aes(x = UserName.x, y = food_mean_beta_div, fill = UserName.x), fill = "orange", outlier.shape = NA, lwd = .25) +
  geom_jitter(data = plot_food_per_subject, aes(x = UserName.x, y = food_mean_beta_div), shape = 21, fill = NA, size = 0.5, width = 0.2, alpha = 0.5) +
  theme_classic() +
  theme(axis.title.x = element_text(size = 9), 
        axis.text.y = element_blank(),
        axis.ticks.y = element_blank(),
        legend.position = "none") +
  labs(y = "Food variability (Unweighted Unifrac)", x = " ") +
  annotate("text", x = 1.5, y = 0.25, label = paste("r =",signif(mytest$estimate, 1), ",", "pval =", signif(mytest$p.value, 1)), size = 2)+
  coord_flip()
...
```{r microbiome div per subject, echo = F, include = F}
...
tax_div <- ggplot()+
  geom_boxplot(data = plot_tax_per_subject, aes(x = UserName.x, y = mean_beta_div, fill = UserName), fill = "red", outlier.shape = NA, lwd = .25) +
  geom_rect(data = shading, aes(xmin = min, xmax = max, ymin = -Inf, ymax = Inf, fill = factor(col), alpha = 0.1)) + # shade soylent
  scale_fill_manual(values = c("white", "grey")) +
  geom_smooth(data = plot_tax_per_subject, aes(x = as.integer(UserName.x), y = mean_beta_div), method = "loess", color = "black") +
  geom_boxplot(data = plot_tax_per_subject, aes(x = UserName.x, y = mean_beta_div, fill = UserName), fill = "red", outlier.shape = NA, lwd = .25) +
  geom_jitter(data = plot_tax_per_subject, aes(x = UserName.x, y = mean_beta_div), shape = 21, fill = NA, size = 0.5, width = 0.2, alpha = 0.5) +
  theme_classic() +
  theme(axis.title.x = element_text(size = 9),
        axis.title.y = element_text(size = 9),
        axis.text.y = element_text(size = 6),
        legend.position = "none") +
  labs(y = "Microbiome stability (CLR-Euclidean)", x = "") +
  coord_flip()
...
mytest <- cor.test(mean_food_test$`median(PD_whole_tree)`, mean_food_test$ave_cloud, method = "spearman")
...
# plot taxonomy diversity
alpha_food <- ggplot()+
  geom_boxplot(data = plot_food_alpha, aes(x = UserName, y = PD_whole_tree, fill = UserName), fill = "orange", outlier.shape = NA, lwd = .25) +
  geom_rect(data = shading, aes(xmin = min, xmax = max, ymin = -Inf, ymax = Inf, fill = factor(col), alpha = 0.1)) + # shade soylent
  scale_fill_manual(values = c("white", "grey")) +
  geom_smooth(data = plot_food_alpha, aes(x = as.integer(UserName), y = PD_whole_tree), method = "loess", color = "black") +
  geom_boxplot(data = plot_food_alpha, aes(x = UserName, y = PD_whole_tree, fill = UserName), fill = "orange", outlier.shape = NA, lwd = .25) +
  geom_jitter(data = plot_food_alpha, aes(x = UserName, y = PD_whole_tree) ,shape = 21, fill = NA, size = 0.5, width = 0.2, alpha = 0.5) +
  ...
  theme(axis.title.x = element_text(size = 9), 
        ...
        axis.ticks.y = element_blank(),
        legend.position = "none") +
  labs(y = "Food diversity (PD-whole-tree)", x = "") +
  annotate("text", x = 1.5, y = 125, label = paste("r =",signif(mytest$estimate, 1), ",", "pval =", signif(mytest$p.value, 1)), size = 2)+
  coord_flip()
...
# no relationship between alpha div and micorbiome stability, don't include in plot.

```


```{r species_stability, echo = F, include = T, fig.height=4, fig.width=6}
...
for (i in colnames(plot[3:ncol(plot)])) {
  ...
  labels = data.frame(x = max(plot[,i])-2, y =min(plot[,"ave_cloud"])+0.04, 
  ...
  labels$xlab <- gsub("_", " ", labels$xlab)
   ...
   print(ggplot(data = plot, aes_string(x = i, y = "ave_cloud")) + 
           ...
           geom_point(aes(fill = UserName), color = "red", size = 3) +
...
```{r figure_4, echo = F, message = F, fig.width=11, fig.height=6}

plot_grid(tax_div, alpha_food, food_div, labels = "AUTO", ncol = 3, align = "h", rel_widths = c(1.0,1.0, 1.0))

```


```{r diet_micro_stability, echo = F, fig.height = 4, fig.width=6}
...
for (i in colnames(plot[3:ncol(plot)])) {
  rho <- paste("italic(r) == ", round(out[i,"cors"], 2))
  q <- paste("italic(qval) ==", round(out[i,"qvals"], 2))
  labels = data.frame(x = (max(plot[,i])-(0.2*(max(plot[,i])))), y =mean(plot[,"ave_cloud"]), 
                       label = paste(rho, q, sep = " ~ "))
  labels$xlab <- i
  labels$xlab <- gsub("_", " ", labels$xlab)
   ...
   print(ggplot(data = plot, aes_string(x = i, y = "ave_cloud")) + 
           geom_smooth(method = "lm", color = "black", fill = "grey") +
           geom_point(aes(fill = UserName), color = "orange", size = 3) +
           geom_point(aes(fill = UserName), shape = 1, size =3, color = "black") +
           theme(legend.position = "none") +
           geom_text(data = labels, aes(x=x, y=y,
            label = label),

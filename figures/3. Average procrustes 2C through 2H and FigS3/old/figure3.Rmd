...
Figure 3: Will show the beta diversity plots for microbiome, diet, and function and the procrustes between diet and microbiome, diet and nutrients, and diet and function.
...
```{r all_tax, echo = F, include = F, fig.height = 2.5, fig.width =2.5}
...
tax_all_plot_one_color <- ggplot(tax_all_pcoa, aes(x = Axis.1, y = Axis.2, fill = UserName)) +
  geom_point(color =  "#5f86b7", alpha=0.75, size = 2.5, alpha = 0.6) +
...
```{r all_food, echo = F, include = F, fig.height = 2.5, fig.width =2.5}
...
food_all_plot_one_color <- ggplot(food_all_pcoa, aes(x = Axis.1, y = Axis.2, fill = UserName)) +
  geom_point(color = "#5a2071", alpha=0.75, size = 2.5, alpha = 0.6) +
  stat_ellipse(color = "dark grey", type = "norm", linetype = 2, level = 0.95, alpha = 0.5) +
  xlab(paste0("PC 1 [",percent_var[1],"%]")) +
  ylab(paste0("PC 2 [",percent_var[2],"%]")) +
  theme_classic() +                                                                      
...
```{r all_func, echo = F, include = F, fig.height = 2.5, fig.width =2.5}
...
## PERMANOVA
mytest <- adonis(func_all_dist~func_all_pcoa$UserName, permutations = 999)
plot_p <- mytest$aov.tab$`Pr(>F)`[1]
...
```{r food_micro_pro, echo = F, include = F, fig.height=2.5, fig.width=2.5}
# make pcoas 
...
beta_pro$type <- "3 day food (Unweighted Unifrac)"
...
food_micro <- ggplot(plot) +
  ...
  annotate("text", x = 0.3, y = -0.27, label = paste0("p-value=",pval), size = 3) +
  xlab(paste0("PC 1 [",percent_var[1],"%]")) +
  ylab(paste0("PC 2 [",percent_var[2],"%]")) 
...
```{r no_tree_food_micro_pro, echo = F, include = F, fig.height=2.5, fig.width=2.5}
# make pcoas 
...
nt_micro <- ggplot(plot) +
  ...
  scale_color_manual(values = c("#02111D", "#5f86b7")) +
    theme_classic() +
    geom_line(aes(x= Axis.1, y=Axis.2, group=UserName), col = "darkgrey", alpha = 0.6) +
  theme(panel.background = element_blank(),
        panel.grid.major = element_blank(),
        ...
        axis.title = element_text(size=9),
         aspect.ratio = 1) +
  guides(color = guide_legend(ncol = 1)) +
  annotate("text", x = -70, y = -65, label = paste0("p-value=",pval), size = 3) +
  xlab(paste0("PC 1 [",percent_var[1],"%]")) +
  ylab(paste0("PC 2 [",percent_var[2],"%]")) 
...
```{r nutr_micro_pro, echo = F, include = F, fig.height = 2.5, fig.width = 2.5}
# make pcoas 
...
nutr_micro <- ggplot(plot) +
  ...
  theme(panel.background = element_blank(),
        ...
        axis.title = element_text(size=9),
         aspect.ratio = 1) +
  guides(color = guide_legend(ncol = 1)) +
  annotate("text", x = -0.1, y = -0.22, label = paste0("p-value=",pval), size = 3) +
  xlab(paste0("PC 1 [",percent_var[1],"%]")) +
  ylab(paste0("PC 2 [",percent_var[2],"%]")) 
...
```{r jacc_nutr_micro_pro, echo = F, include = F, fig.height = 2.5, fig.width = 2.5}
# make pcoas 
pcoa_n <- jacc_summ_pcoa
pcoa_t <- as.data.frame(pcoa(tax_dist)$vectors)
...
trans_pro <- data.frame(pro$Yrot)
beta_pro$UserName <- rownames(beta_pro)
beta_pro$type <- "Nutrient (Jaccard)"
trans_pro$UserName <- rownames(trans_pro)
trans_pro$type <- "Microbiome (Aitchison's)"
...
nutr_micro <- ggplot(plot) +
  geom_point(size = 4, alpha=0.75, aes(x = Axis.1, y = Axis.2, color = type)) +
  scale_color_manual(values = c("#5f86b7", "pink")) +
  theme_classic() +
    geom_line(aes(x= Axis.1, y=Axis.2, group=UserName), col = "darkgrey", alpha = 0.6) +
  theme(panel.background = element_blank(),
        panel.grid.major = element_blank(),
        ...
        axis.title = element_text(size=9),
         aspect.ratio = 1) +
  guides(color = guide_legend(ncol = 1)) +
  annotate("text", x = 0.0, y = -0.33, label = paste0("p-value=",pval), size = 3) +
  xlab(paste0("PC 1 [",percent_var[1],"%]")) +
  ylab(paste0("PC 2 [",percent_var[2],"%]")) 
...
```{r nutr_food_pro, echo = F, include = F, fig.height = 2.5, fig.width = 2.5}
# make pcoas 
pcoa_n <- as.data.frame(pcoa(nutr_dist)$vectors)
pcoa_f <- as.data.frame(pcoa(food_dist)$vectors)

# procrustes
pro <- procrustes(pcoa_n, pcoa_f)
pro_test <- protest(pcoa_n, pcoa_f, perm = 999)
...
beta_pro$type <- "Nutrient (Euclidean)"
trans_pro$UserName <- rownames(trans_pro)
trans_pro$type <- "Food (Unweighted Unifrac)"
...
nutr_micro <- ggplot(plot) +
  geom_point(size = 3, alpha=0.75, aes(x = Axis.1, y = Axis.2, color = type)) +
  scale_color_manual(values = c("#5a2071", "#2BBAA7")) +
    theme_classic() +
    geom_line(aes(x= Axis.1, y=Axis.2, group=UserName), col = "darkgrey", alpha = 0.6) +
  theme(panel.background = element_blank(),
        panel.grid.major = element_blank(),
...
```
```{r nutr_jacc_food_pro, echo = F, include = F, fig.height = 4, fig.width = 4}
# make pcoas 
pcoa_n <- jacc_summ_pcoa
pcoa_f <- as.data.frame(pcoa(food_dist)$vectors)
...
nutr_micro <- ggplot(plot) +
  ...
  scale_fill_manual(values = c("#5dd047", "#2BBAA7")) +
  theme_bw() +
 ...
 theme(panel.background = element_blank(),
        ...
        axis.title = element_text(size=9),
         aspect.ratio = 1) +
  annotate("text", x = -0.1, y = -0.22, label = paste0("p-value=",pval)) +
  xlab(paste0("PC 1 [",percent_var[1],"%]")) +
  ylab(paste0("PC 2 [",percent_var[2],"%]")) 
...
```
```{r func_food_pro, echo = F, include = F, fig.height = 2.5, fig.width = 2.5}
# make pcoas 
...
trans_pro$UserName <- rownames(trans_pro)
trans_pro$type <- "Function (Aitchison's)"
...
func_food <- ggplot(plot) +
  geom_point(size = 3, alpha=0.75, aes(x = Axis.1, y = Axis.2, color = type)) +
  scale_color_manual(values = c("#5a2071", "pink")) +
    theme_classic() +
    geom_line(aes(x= Axis.1, y=Axis.2, group=UserName), col = "darkgrey", alpha = 0.6) +
  theme(panel.background = element_blank(),
        panel.grid.major = element_blank(),
        ...
        axis.title = element_text(size=9),
         aspect.ratio = 1) +
  guides(color = guide_legend(ncol = 1)) +
  annotate("text", x = 0.23, y = 0.25, label = paste0("p-value=",pval), size = 3) +
  xlab(paste0("PC 1 [",percent_var[1],"%]")) +
  ylab(paste0("PC 2 [",percent_var[2],"%]")) 
...
mytest <- t.test(plot$cors.mb, plot$mean_mc.cors.mb, paired = T)
pval <- signif(mytest$p.value,2)
...
pre <- ggplot() +
  ...
  annotate("text", x = 25, y = 0.4, label = paste0("p-value=",pval), size = 3.5)
...
tax_sum_plot <- ggplot(tax_sum_pcoa, aes(x = Axis.1, y = Axis.2, fill = UserName)) +
  geom_point(color = "black", alpha=0.75, pch = 21, size = 4, alpha = 0.5) +
...
food_sum_plot <- ggplot(food_sum_pcoa, aes(x = Axis.1, y = Axis.2, fill = UserName)) +
  ...
  theme(legend.position = "right",
        ...
        axis.text = element_text(size = 9)) +
  guides(color = F, fill = guide_legend(ncol = 2)) 
  #theme(legend.title = element_text("Subject"))

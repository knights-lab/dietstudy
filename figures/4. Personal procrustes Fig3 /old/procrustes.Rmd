...
Goal of this notebook is to generate a data for downstream procrustes analysis on a per-person basis in QIIME. 
...
```{r load food data, echo = FALSE}
food <- read.table("data/processed_food/dhydrt.txt", header = T, sep = "\t", comment = "")
food_taxa <- read.table("raw/diet.taxonomy.txt", header = T, sep = "\t", comment = "")
rownames(food_taxa) <- food_taxa$Main.food.description
...
for (i in unique(map_sample$UserName)){
  ...
  # foods
  foodsub[[i]] <- food[,colnames(food) %in% subgroup$X.SampleID, drop = F] # subset to one username
  ...
  #rename the new summarized data starting with the thrid day's name
  ...
  colnames(foodsub[[i]])[1] <- "#FoodID"                         # add name
  foodsub[[i]] <- foodsub[[i]][,!colnames(foodsub[[i]]) %in% c("Main.food.description", "FoodID")]
  ...
  # map
  mapsub[[i]] <- subgroup[subgroup$X.SampleID %in% colnames(foodsub[[i]]), drop = F] # subset to match the foodsub ids
...
for (df in names(mapsublist)) {
  #write foods to table
  write.table(foodsublist[[df]], file = paste0("analysis/procrustes/data_username_offset/", df, "_food", ".txt"), sep = "\t", quote = F,  row.names = F, col.names = T)
  
  # write taxa to table
  write.table(taxsublist[[df]], file = paste0("analysis/procrustes/data_username_offset/", df, "_tax", ".txt"), sep = "\t", quote = F,  row.names = F, col.names = T)
  
  # write map to table
  write.table(mapsublist[[df]], file = paste0("analysis/procrustes/data_username_offset/", df, "_map", ".txt"), sep = "\t", quote = F,  row.names = F, col.names = T)

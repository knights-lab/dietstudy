...
opts_knit$set("/Users/abby/Documents/Projects/dietstudy/")
opts_chunk$set(echo = TRUE, fig.path = "Figs_offset/", dev = c("png", "pdf"), dpi = 300)
set.seed(7)
...
```{r data, include =F, warning=F, message= F}

source("lib/load_data_lists_offset.R")

```

```{r pro1, echo = F, include = F, fig.height = 4, fig.width = 4, warning=F, message = F}
...
```{r pro2, echo = F, include = F, fig.height = 4, fig.width = 4, warning=F, message = F}
...
```
```{r pro3, echo = F, include = F, fig.height = 4, fig.width = 4, warning=F, message = F}
...
```{r multiplot_func, echo=F, fig.height=2.7, fig.width=7}
...
```{r pro4, echo = F, include = F, fig.height = 4, fig.width = 4, warning=F, message = F}
...
```{r pro5, echo = F, include = F, fig.height = 4, fig.width = 4, warning=F, message = F}
...
```{r pvals_boxplot_combined_days, echo = F, fig.width=4, fig.height = 2.5}
...
myplot <- ggplot(plot_melt, aes(x = variable, y = -log(value))) +
  ...
  guides(color = guide_legend(ncol = 3, title = "Subject", title.position = "top")) +
  xlab(label = "Offset of days prior to sample") +
  ylab(label = "-log(Monte Carlo p-value)")
...
```
```{r pvals_boxplot_legend, fig.height = 2.5, fig.width=1.2}

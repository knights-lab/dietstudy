---
title: "Microbiome function visualization"
author: "Abby"
...
mydist <- vegdist(t(microbes), method = "bray")
...
taxa <- read.table("data/shogun_results_cleaned/shogun_no_dropouts/modules.txt", header = T, row.names = 1, sep = "\t", comment = "")
...
```{r function_subject_day_wide, echo = F, fig.height=4, fig.width=15.5}
set.seed(11)
...
plot <- merge(plot, path_map, by.x = "Species", by.y = "Shogun_ID")
plot$KEGG_Label <- gsub(" \\[.*$", "", plot$KEGG_Label)
plot$KEGG_Label <- gsub("M..... ", "", plot$KEGG_Label)


# make the plot
myplot <- ggplot(plot, aes(x = StudyDayNo, y = newvalue, fill = KEGG_Label)) +
 #facet_grid(UserName~.) +
  facet_grid(.~UserName) +

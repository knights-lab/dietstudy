...
tax_alpha <- read.delim("data/processed_tax/taxa_alpha.txt", sep = "\t", row.names = 1)
...
micro_order <- tax_alpha %>% group_by(UserName) %>% dplyr::summarize(ave = mean(chao1, na.rm=T))
# sort by mean
...
```{r plotdietmicrobiome, echo = F, message = F, fig.height=3, fig.width=11}
...
labels = data.frame(x =150, y =30, label = corr_eqn(myplot$Food_alpha, myplot$Tax_chao1))
...
tax_ra <- read.table("data/processed_tax/taxonomy_norm.txt", header = T, row.names = 1, sep= "\t", comment = "")

# get average relative abunance for each individual
#initiate the dataframe with the correct information
tax_subject <- data.frame(matrix(nrow=nrow(tax_ra), ncol=0))
rownames(tax_subject) <- rownames(tax_ra)

#loop through and summarize by username
for (i in unique(tax_map$UserName)){
  subgroup <- tax_map[tax_map$UserName == i,]
  sub <- tax_ra[(colnames(tax_ra) %in% subgroup[,"X.SampleID"])]
...
pvals <- apply(tax_subject, 1, function(x) {cor.test(x, mytaxalpha$`mean(chao1)`, method = "spearman", exact = F)$p.value})
cors <- apply(tax_subject, 1, function(x) {cor(x, mytaxalpha$`mean(chao1)`, method = "spearman")})
...
# load taxa relative abundances
food <- read.table("data/processed_food/dhydrt.otu.txt", header = T, row.names = "taxonomy", sep= "\t", comment = "")

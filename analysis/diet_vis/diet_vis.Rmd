...
food_dist <- read.delim("data/processed_food/dhydrt_smry_beta/weighted_unifrac_dhydrt.smry.otu.txt", sep = "\t", row = 1)
food_dist <- as.dist(food_dist)
myclust <- hclust(food_dist, method = "average")
...
```
There are three dietary clusters when we consider all the foods consumed by participants in our study (4 if you count the Soylent outliers).
...
```{r food_subject_day_wide, echo = F, fig.height=6, fig.width=15.5}
...
# make the plot
myplot<-ggplot(plot, aes(x = StudyDayNo, y = newvalue, fill = Food)) +
  #facet_grid(UserName~.) +
  facet_grid(.~UserName) +

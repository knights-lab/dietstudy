---
title: "Targeted analysis of fiber from Vegetables"
author: "Abby"
...
Hypothesis: Intake of fiber from vegetables is correlated with microbime beta diversity.
...
vege <- fiber %>% slice(grep("L1_Vege", fiber$taxonomy))

# remove any grains with rowsums = 0
keep <- rowSums(vege[,2:32])>0
vege<- vege %>% filter(keep)
...
colnames(vege)[1] <- "#FoodID" 
write.table(vege, "data/fiber/vege_data/vege_fiber.txt", sep = "\t", row.names = F, quote = F)
...
vege <- ggplot(plot, aes(x = Axis.1, y = Axis.2)) +
  ...
  labs(title="Uweighted Unifrac Vegetable Fiber") +
...
food_micro <- ggplot(plot) +
  ...
  scale_fill_manual(values = c("#E71D36", "#5DD047")) +
  theme_bw() +
...
alpha <- read.delim(file = "data/fiber/vege_data/vege_alpha.txt", sep = "\t")
...
ggplot(test, aes(y = (1/ave_cloud), x = PD_whole_tree)) + 
  ...
  geom_point(aes(fill = UserName), color = "#5DD047", size = 3) +
  ...
  geom_text(data = labels, aes(x=x, y=y,
                               label = label),
            parse = T, size = 3) +
  labs(x = "Vegetable fiber alpha diversity\n(PD whole tree)", y= "Microbiome Stability")

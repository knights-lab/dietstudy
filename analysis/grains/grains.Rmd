...
Hypothesis: Intake of fiber from grain is correlated with microbime beta diversity.
...
3. Plot pcoa
4. Start with taxonomy counts UN_taxonomy_counts_s.txt
5. beta diversity of counts table by putting it in qiime to get a beta diversity matrix. It is shotgun data, so no tree for unifrac. Need to do chi-squared and Bray-Curtis.
6. Run transformed coordinate analyses in procrustes in qiime. Use qiime website to find example code.
...
write.table(grains, "data/fiber/grains_data/grains_fiber.txt", sep = "\t", row.names = F, quote = F)
...
Grain <- ggplot(plot, aes(x = Axis.1, y = Axis.2)) +
  ...
  labs(title="Uweighted Unifrac Grain Fiber") +
...
food_micro <- ggplot(plot) +
  ...
  annotate("text", x = 0.1, y = -0.27, label = paste0("p-value=",pval), size = 3) +
...
# better ways to visualize this - make a box plot of the lengths v. permuted lengths?
```



```{r diet alpha v. stability, echo = F, fig.height=4, fig.width=4}
# load grains alpha diversity
grains_alpha <- read.delim(file = "data/fiber/grains_data/grains_alpha.txt", sep = "\t")
colnames(grains_alpha)[1] <- "UserName"

beta_cloud <- read.delim(file = "../dietstudy/analysis/beta_diversity/beta_div_cloud.txt", sep = "\t")

# food alpha v. beta cloud?
test <- merge(beta_cloud, grains_alpha)
...
ggplot(test, aes(y = (1/ave_cloud), x = PD_whole_tree)) + 
  ...
  geom_point(aes(fill = UserName), color = "#fe9700", size = 3) +
...
fiberStrings = sapply(split,function(x) paste(x[1:2],collapse=";"));
...
for(i in colnames(x)){ 
  for(j in colnames(y)){
    a<-as.numeric(x[,i]) 
    b<-as.numeric(y[,j])
    tmp <- c(i,j,cor(a,b, method = "spearman", use = "pairwise.complete.obs"), 
             cor.test(a,b,method = "spearman", use = "pairwise.complete.obs", exact = F)$p.value)
    if(is.null(dat)){
      dat <- tmp
    }
    else{
...
ggplot(data = dat.m, aes(x=Food, y=Taxa, fill=Correlation)) + 
  ...
  labs(x = "Fiber Grain Source", y = "Family") +

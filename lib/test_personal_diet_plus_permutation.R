...
most.common <- names(head(sort(mb.means, decreasing = T), n = 25))
...
"test.diet.impact" <- function(){
  ...
  for (k in 1:length(dat)) {
    ...
    for (i in seq_along(tomorrow.species)) {
      ...
      for (j in 1:10000) { # change here to adjust permutations
...
save(myresults, file ='output/permutation.results.Rdata')

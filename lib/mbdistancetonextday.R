...
myplot <- ggplot(data = plot, aes(x = mb_dist_to, y = distance)) + 
  geom_jitter(alpha = 0.2, width = 0.35, pch = 21, fill = "#5f86b7") +
  geom_boxplot(outlier.shape = NA, fill = NA) +
  annotate("text", x = 1.5, y = 35, label = "italic(p) < 0.001", size = 3, parse = T) +
  theme_classic() +
...
ggsave("figures/mbdistancetonextday.pdf", myplot, height = 2.5, width = 2.5, device = "pdf")
